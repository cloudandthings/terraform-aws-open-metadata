...
| <a name="input_opensearch_instance_count"></a> [opensearch\_instance\_count](#input\_opensearch\_instance\_count) | OpenSearch data node count. Any value above 1 enables zone awareness. Three or more nodes use three availability zones when at least three subnets are available, otherwise two, which requires an even count. Multiples of 3 spread nodes evenly across three zones. | `number` | n/a | yes |
