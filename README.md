...
| <a name="input_opensearch_instance_count"></a> [opensearch\_instance\_count](#input\_opensearch\_instance\_count) | OpenSearch data node count. Any value above 1 enables zone awareness, so the count must be a multiple of 2 or 3 to spread nodes evenly across availability zones. | `number` | `2` | no |
